...
public class OmokImpl implements OmokInterface {
	public int[][] omok = new int[18][18];
	// 오목판 크기 x, y 전체 좌표
	
	// 오목판 출력 함수 
	public void viewOmok() {		
		int numY = 0;				// 오목 좌측 좌표값 초기화
		int numX = 0;				// 오목 하단 좌표값 초기화
	...
	public int OmokAction(int x, int y, int turn) {
		...
		while(omok[_y++][_x++] == turn && _y < 18 && _x < 18) {
			count++;
		...
		while(omok[_y-- <= 0 ? 0 : _y][_x++] == turn && _y < 18 && _x < 18) {
	...
	public int getomok(int x, int y) {
			return omok[y][x];
	}
